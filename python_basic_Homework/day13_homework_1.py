...
from day12_homework_1 import qytang_multicmd    #day12_homework_1.py中，需要把if verbose和下面的两行print注释掉，否则会先打印day12的结果。
...
class Interface:
    ...
    def __str__(self):
        """格式化打印接口信息"""
        status_str = "no shutdown" if self.status else "shutdown"  # 如果 self.status 为 True 则为 'no shutdown'，否则为 'shutdown'
        device_ip = self.device.ip if self.device else "未绑定设备"   # 如果 self.device 存在则取 self.device.ip，否则显示 '未绑定设备'
        lines = [
            f"接口名称    : {self.name}",
            f"所属设备    : {device_ip}",
...
if __name__ == '__main__':
    ...
    r1 = NetworkDevice('10.10.1.1', 'admin', 'Metax@123')
